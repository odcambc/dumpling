...
# ===========================
#   RUN ROSACE-AA ANALYSIS
# ===========================
run_rosace_for_condition <- function(rosace_obj, experiment_name, expt_condition, noprocess) {
  # Run rosace-aa for one condition and write its scores CSV.
  #
  # rosace-aa's entrypoint is also named RunRosace (verified against
  # pimentellab/rosace-aa R/runROSACE.R), but its Rosace-class method takes
  # additional metadata args over rosace's: wt.col, mut.col, aa.code,
  # stop.col, stop.name, plus optional pos.act / blosum.label. These
  # reference columns in the variant metadata produced by process_variants
  # — see the audit notes in tasks/tasks.md for the mapping rationale.
  if (is.null(rosace_obj)) {
    stop("Rosace object is NULL; cannot run RunRosace.")
  }

  rosace_dir <- file.path("results", experiment_name, "rosace_aa")
  message(sprintf("Running rosace-aa for condition: %s", expt_condition))
  # Stan seed for rosace-aa is hardcoded to 100 in MCMCRunStan upstream
  # (same as rosace; verified against pimentellab/rosace-aa
  # R/runROSACE.R). Surface for parity with run_lilace.R's seed logging.
  message(
    "rosace-aa Stan seed: 100 (hardcoded in rosaceAA::MCMCRunStan upstream). ",
    "rosace-aa runs are bit-identical across reruns by construction; ",
    "for configurable seeds, use scoring_backend='lilace' with the ",
    "lilace_seed config knob."
  )

  # noprocess is rejected at config-validation time for rosace-aa
  # (validate_scoring_backend_mode): wt.col / mut.col / ctrl.col all
  # require process_variants metadata. The noprocess branch from
  # run_rosace.R is intentionally absent here — if it ever fires we have
  # a config-validator bug.
  if (noprocess) {
    stop(
      "scoring_backend='rosace_aa' is incompatible with noprocess=true ",
      "(should have been rejected by validate_scoring_backend_mode). ",
      "Use scoring_backend='rosace' for noprocess runs."
    )
  }
...
tryCatch(
  main(),
  error = function(err) {
    log_and_rethrow("run_rosace_aa", err)
  }
)
